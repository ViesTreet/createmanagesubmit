...
cv2.setNumThreads(0)  # usa todos los cores
...
class DocScanner(object):
    ...
    def get_contour(self, rescaled_image):
        ...
        # ---- PLAN B: LSD SOLO SI NO HAY CONTORNOS ----
        if not approx_contours:
            test_corners = self.get_corners(edged)

            if len(test_corners) >= 4:
                test_corners = test_corners[:8]
                quads = []
    ...
    def scan(self, image_path):
        ...
        # ===== Mejora suave manteniendo color =====

        # aumentar contraste sin quemar blancos
        ...
        final = cv2.addWeighted(enhanced, 1.2, blur, -0.2, 0)

        basename = os.path.basename(image_path)
        cv2.imwrite(OUTPUT_DIR + '/' + basename, final)
